...
### PHASE 11 — NESTED CREATIONS, ROOM MUSIC & OBJECTS (IN PROGRESS — milestone 1 of 5 ✅ complete, verified live 2026-07-19)
...
- **Known gap, deliberately not built this pass:** the "Movable" checkbox in the Add Object form only tags an object server-side — there's no drag-to-move client UI wired to `POST /api/objects/:id/move` yet. That's the remainder of milestone 2.
- Not yet built: `music`/interactive-`object` creation-kind files, the admin-panel filter options for them, `src/objects/_template.js`, and room-music playback wiring — milestones 3–4 per the Phase 11 plan (saved at the time of writing in a Claude Code plan file, not yet copied into this brief in full detail — re-derive from the "Build order" section if picking this up in a fresh session without that plan file).
