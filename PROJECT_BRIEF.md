...
### PHASE 12 — CREATIVE PROCESS LOG (✅ COMPLETE, verified live in-browser by the owner 2026-07-21)

Goal: past/present/future reflection on a room's whole creative package, editable by the room's owner, visible to anyone viewing the room.

**Scope decision (2026-07-21):** narrowed from the original "per-creation" wording to **one Process Log per room** (not a separate log per room/game/music/object) — the owner's explicit call: this covers "the whole room's creative experience as a package," not four fragmented logs. Games, music, and objects don't get their own logs. Visibility: anyone in-world can view; only the room's owner can edit. No version history on the log itself (single mutable past/present/future snapshot, overwritten in place — distinct from Phase 13's future code-version-history, which tracks the creation's *code*, not this text). No admin moderation — plain text, never touches the pending-queue/approval path.

- New additive Firestore collection `processLogs/{slotKey}` (one doc per room slot, not a `creations/{id}` migration — that bigger migration stays Phase 13's concern), with an in-memory `processLogsCache` write-through-cache exactly mirroring the `objectsCache`/`persistObject` pattern from Phase 11.
- `GET /api/process-log?slotKey=` (public, no auth) and `POST /api/process-log` (idToken-verified, 403 unless `uid` matches the room's owner) added to `server/index.js`, same auth/ownership pattern as the `/api/objects/*` routes.
- Client: `[P]` key in `RoomScene.js` opens a DOM-overlay popup (same convention as the `[H]` Help window) with three Past/Present/Future fields — editable textareas + Save button for the room owner, read-only textareas for visitors. Hint text `[P] Process Log` always visible bottom-right, no proximity gate (reflects the whole room, like Help). Also added a short mention to the Help window's first page for discoverability.
...
- **Verification status: ✅ fully verified live in-browser by the owner.** Server-side verified via `curl` first (invalid-slot GET → 400, valid-slot GET → empty defaults, POST without idToken → 400, POST with a bad idToken → 401). **Two real bugs found during live in-browser testing, both fixed and confirmed resolved:**
  1. **`[P]` toggle conflicted with typing.** The original toggle (`[P]` opens, `[P]` again closes — mirroring `[H]` Help) broke because Phaser's Key objects keep tracking keydowns globally even while the browser sends them to a focused DOM textarea (`disableGlobalCapture()` only suppresses `preventDefault()`, not Phaser's own key-state tracking) — so typing any word containing "p" ("past", "present", "process") closed the overlay mid-reflection. Fixed by moving the `[P]` check to open-only, placed after the movement-freeze early-return in `update()` (same position/convention already used by the Music/Object/Game overlays for this exact reason) — closing is now only via the Close button or Esc.
  2. **Stale orphaned overlay DOM element on re-entry/re-login.** `create()` already has a stale-overlay cleanup block (`document.getElementById('woc-*-overlay')?.remove()`, see the comment at `RoomScene.js:343-348`) that removes leftover DOM elements from a *previous* RoomScene instance before creating a new one — because `scene.start()` never destroys the old `document.body`-appended overlay divs, and `getElementById()` silently returns the *first* matching id in the DOM if a stale duplicate is left behind, binding all the field/button lookups to the dead, hidden copy instead of the new visible one. The Process Log overlay was never added to that cleanup list, so logging out and back in (or leaving and re-entering a room) left an invisible orphaned copy that any new overlay's `getElementById` calls would bind to instead of the real one — making the visible popup appear permanently empty regardless of what was actually saved server-side. (Confirmed server-side data was never actually lost — `slot45`'s log was intact in `processLogsCache` throughout the whole debugging session.) Fixed by adding `woc-processlog-overlay` to the same cleanup block.
  - Owner re-ran the full repro after both fixes (typing a "p"-word reflection, save, close via button, reopen, log out/in without a hard reload, view as a different account) and confirmed everything works correctly.

---

### PHASE 13 — SHARING & REMIX + VERSION HISTORY (✅ COMPLETE, verified live in-browser by the owner 2026-07-21)
...
**Scope decision:** applies to `room`, `game`, `music`, and interactive `object` — the four kinds that flow through `/api/submit`/`/admin/approve`. Decorative objects are explicitly out of scope: they're pure JSON, auto-approved through a completely separate endpoint pair, and never touch the code-validation pipeline this phase's version history hooks into.

- New additive Firestore collection `creationMeta/{creationKey}`, with an in-memory `creationMetaCache` write-through cache mirroring the `objectsCache`/`processLogsCache` pattern from Phases 11–12 exactly. Since there's no unified `creations/{id}` collection in this codebase (room/game/music live as files + `slotAssignments` fields, objects live as their own Firestore docs), `creationKey` is derived per kind: bare `slotKey` for room, `` `game:${slotKey}` ``/`` `music:${slotKey}` `` for game/music, and the object's own doc id for interactive objects.
- Each version entry stores the full `code` (not just metadata) — disk only ever holds the *current* live file (unchanged from before this phase), so Firestore is the only place old code can survive for a "version history" to mean anything.
- `server/index.js`: `/api/submit` gained a version-cap check (rejects a submission with 400 if the target already has 5 versions and no valid `dropVersionId` is supplied); `/admin/approve` gained a unified post-write block (after the existing per-kind file-write/`slotAssignments`/`objectsCache` logic, unchanged) that evicts the chosen version if at the cap and appends the new one. Three new endpoints: `GET /api/creation-meta` (public, strips `code`), `GET /api/creations/shared` (public, cross-kind list feeding the remix picker), `POST /api/creation-meta/share` (idToken + ownerUid-gated toggle, 404 if nothing's been approved for that creationKey yet — independent of resubmitting code, since flipping a boolean shouldn't force a full re-review).
- Client: new shared helper module `src/engine/creationSharing.js` (justified as shared, not duplicated 4×, since it's the exact same feature at every call site) provides the fetch wrappers and DOM-builder helpers wired into all 4 existing code-submission overlays — room claim (`WorldScene.js`), game/music/interactive-object (`RoomScene.js`). Each overlay gained: a "describe this version" field (copy branches on whether the target already has a version — "Describe this creation" vs. "What changed?"), an optional "based on" dropdown populated from currently-shared creations across all kinds, and (room/game/music only — objects have no way to reopen an already-approved creation this milestone, so there's nowhere to toggle sharing after the fact) a "share this code" checkbox that fires immediately on change, independent of code submission. A version-cap picker (radio list over the existing 5 versions) only renders when the fetched meta shows the cap is already reached, and blocks Submit until a version is chosen.
- `admin.html`: `loadPending()`'s card rendering gained badge lines for `versionDescription`/`remixedFrom`/`dropVersionId` — no server changes needed since `meta` already passed through the pending-submission response as-is; no new admin actions, since sharing/remix/version choice are never admin-moderated (only the code itself still goes through the existing validate/approve/reject gate).
- **Verified via `curl` against a live server boot:** 5 sequential room-update approvals correctly built a 5-entry version history; a 6th submission without `dropVersionId` correctly 400s (and again with an invalid `dropVersionId`); a 6th submission with a valid `dropVersionId` correctly evicted the chosen version and appended the new one, holding at 5 total; a `game` submission citing `remixedFrom` correctly stored and surfaced it under its own `game:${slotKey}` creationKey; an interactive-object approval correctly created a creationMeta doc keyed by the object's own id (confirmed indirectly — a share-toggle attempt against it returned 401 invalid-token rather than 404 not-found); the share-toggle endpoint correctly 400s without an idToken and 404s for an unknown creationKey. All test data (slot assignments, on-disk files, Firestore docs) was cleaned up after verification — `git diff` on `server/data/slots.json` is empty. `npx vite build` and `node --check` clean throughout.
- **Live in-browser verification (2026-07-21) — room sharing confirmed working end-to-end by the owner:** the version-description field, "based on" remix dropdown, and share checkbox on the room claim/update overlay all behave as designed.
- **Real bug found during live verification and fixed, then confirmed resolved by the owner:** the 4 submission overlays are `position:fixed;inset:0;display:flex` centering an inner card with no `max-height`/`overflow-y` — fine before this phase, but the new fields (description input, "based on" dropdown, share checkbox, and especially the 5-row version-cap picker) pushed the card taller than the viewport with nothing scrollable anywhere in the stack, so Submit became genuinely unreachable once the cap picker appeared. Fixed by adding `max-height:90vh;overflow-y:auto` to all 4 overlay inner-card divs (room claim in `WorldScene.js`; game/object/music in `RoomScene.js`).
- **Explicit scope decision (owner, 2026-07-21): interactive-object code sharing/remix deferred, not built now** — logged in the "Deferred Follow-Ups" section near the end of this file, alongside game/music sharing's still-outstanding live verification (already built and `curl`-verified, owner chose to defer *testing* it rather than removing it). See that section for the full list and reasoning.
...
### PHASE 15 — IN-PERSON SESSION MODE (✅ COMPLETE, verified live in-browser by the owner 2026-07-23)

Goal: admin-started, invite-only temporary world instance for a facilitated group.

- Admin panel: "Start session" — pick a roster of registered players; a new `SessionRoom` Colyseus room type spins up, invite-only via `onAuth` (verifies idToken, checks roster), one instance per session via `.filterBy(['sessionDocId'])`. Deliberately **not** a subclass of `WorldRoom` (its `onCreate` hardcodes the singleton `worldRoomInstance = this`, which must never fire for a session) — shared join/leave/movement logic lives in extracted `registerWorldMessages`/`joinWorldPlayer`/`leaveWorldPlayer` helpers instead.
- Players in a session see only each other; online feedback is implicitly live for the session's duration regardless of the global toggle — a signed-in player's uid being in the active session's roster is re-verified server-side on every `/api/feedback` POST, never trusted from the client alone.
- **Room/game/object/music content is shared, global data, not session-forked** — a `SessionRoom` and the main `WorldRoom` both read the exact same `slotAssignments` map; a session only isolates *players* from each other, never content. So a submission approved while inside a session is immediately visible to everyone everywhere, same single pending-submission queue as always, no special-casing needed.
- "End session" is a **60-second grace period**, not an abrupt cut: admin's click broadcasts a `sessionEnding {endsAt}` countdown to the session's clients (session stays fully `active` throughout — feedback bypass, roster checks, etc. keep working normally) and schedules the real finalize (`finalizeSessionEnd`: mark ended, disconnect the room, auto-archive session-tagged online feedback) 60s later. Each player gets their own countdown banner with a "Return to Main World now" button to leave early and independently. Admin also has a **"Force End Now" veto** (`POST /admin/session/end-now`) that clears the timer and finalizes immediately, whether or not the countdown was ever started — for when 60s isn't fast enough.
- **Players are pulled back to the main world regardless of what they're doing** — mid-room, mid-mini-game (`GameScene` launched on top of a paused `RoomScene`), or with an overlay open (feedback/help/object/music/etc.) — not just when they happen to walk into a portal or manually exit. `RoomScene.forceExit()` closes any open overlay (removing its DOM node, which a normal exit never bothers doing — that's lazily swept up by the *next* room's `create()` instead) and runs the room's real `exitRoom()`; `WorldScene._forceOutOfAnyRoom()` sequences `GameScene` → `RoomScene` → world in the right order. Same helper is used both when a session ends (via `room.onLeave`) and when accepting a session invite while already mid-room in the main world.
- A **portal/warp camera transition** (zoom + fade to a portal-tinted color, via `WorldScene._playPortalTransition()`) plays whenever the player's world swaps out from under them — accepting a live invite, or falling back to the main world when a session ends/is vetoed. A persistent top-left badge ("Main World" / "In-Person Session") always shows which world the player is currently in.
- Admin panel gained a **session-scoped tab** — pending submissions filtered to the active session's roster (by submitter uid; a player can only ever submit into an unclaimed slot or one they already own, per `/api/submit`'s ownership check, so "by submitter" and "by room ownership" collapse to the same thing) and a read-only feedback list scoped to the session, reusing the same approve/reject buttons as the main Pending tab. Rendering the same submission in two tabs at once (tab-panes stay in the DOM, just CSS-hidden) required namespacing every card's element ids by tab (`main-`/`session-` prefixes) to avoid `getElementById` collisions.
- `sessions/{id}` tracks roster + status; feedback entries carry `inPersonSessionId` (renamed from a dead, unused `sessionId` field that collided in name — but not in meaning — with the pre-existing Colyseus-connection `sessionId` used everywhere else in `server/index.js`).
- **Real bugs found during live verification, all fixed:** (1) `broadcastSlotsUpdated()`/`broadcastObjectsUpdated()` only ever broadcast to `worldRoomInstance`, never to any `sessionRoomInstances` — session players needed a manual refresh to see newly-approved content. (2) The client hid the online-feedback form based solely on the global toggle, ignoring session membership, even though the server-side bypass was already correct. (3) Ending a session (or the connection dropping) had no client-side `room.onLeave` handler at all — players were left stuck disconnected. (4) Once (3) was fixed, players stuck inside a room/mini-game/overlay still weren't visually pulled out until they manually exited — the underlying reconnect had happened, but nothing forced the visible scene to close (fixed via `forceExit`/`_forceOutOfAnyRoom` above). (5) The mirror-image bug in the *other* direction: accepting a session invite while mid-room deadlocked, because `_forceOutOfAnyRoom()` was called *inside* the portal transition's completion callback — but `WorldScene` is asleep whenever a room is open, so its camera fade can never actually complete (no update loop stepping) without something else waking it first. Fixed by calling `_forceOutOfAnyRoom()` *before* starting the transition, not after.

---

### PHASE 16 — PROFILE PAGE (✅ COMPLETE, verified live in-browser by the owner 2026-07-24)

Goal: a page showing one player's character, creations, Creative Process Logs, and feedback received — visible to all players, and now the default landing spot after login rather than a side destination.

**Build status:** fully complete and closed out. All pieces below are implemented, the client/server build clean (`vite build`, `node --check`), and every flow has been verified live in-browser by the owner: the landing flow (Login/CharacterScene → ProfileScene → Enter World), the in-world "My Profile" HUD button's pause/launch round-trip (Colyseus connection preserved, no visible reconnect), viewing another player via sprite/nametag click and via the "Players Online" sidebar, the sidebar's in-person-session tagging, and the admin `profileVisible` hide/unhide toggle. Two real UI bugs were found and fixed while iterating on the layout live: (1) the original ground-"glow" behind the character was two overlapping flat Phaser circles, which reads as a visible ring/bullseye rather than a soft light (Phaser's basic shapes have hard edges, no blur) — replaced with a single flat contact-shadow ellipse at the character's feet; (2) the character preview sat too far left relative to the newly-right-docked info panel — `PREVIEW_X`/`PREVIEW_Y` retuned (210→280, 370→380) to better center it in the visible canvas area. Committed as `d9671b7` ("Implement Phase 16: Profile Page"), pushed to `main`.

**Layout note (2026-07-24):** `ProfileScene`'s DOM panel was redesigned from an always-visible stacked-sections left column into a mobile-RPG-style character-profile layout the owner referenced (large character on the left standing in a ground shadow, a translucent panel docked to the right edge with tabs and clean label:value rows, one prominent action button sticky at the panel's bottom) — same structural language, not the reference's art style (our characters stay flat shape-recipes). Tabs: Creations / Process Log / Feedback, swapped instead of scrolled through. Both Creations and Process Log already handled multi-room players correctly by construction (both are arrays keyed by room, logs get a room-name header once there's more than one) — the one real gap found was Feedback not showing *which* room an entry was about when a player owns more than one room; fixed by appending a `· Room Name` suffix to each feedback card, shown only when `rooms.length > 1` so it stays quiet in the common single-room case.

**Architecture note (supersedes the original "outside the Phaser canvas, a normal web route" framing):** this app has no client router — LoginScene/CharacterScene/WorldScene/RoomScene are all Phaser scenes, some using a DOM-overlay-over-canvas hybrid (LoginScene's sign-in card, CharacterScene's live character preview + form). Profile Page follows that same established pattern as a new **`ProfileScene`**, not a separate static page, so it can reuse `CharacterRenderer` directly and sit inline in the existing scene-transition flow.

- **Character skin, alive:** `ProfileScene` renders the player's saved `characterConfig` via the same `CharacterRenderer.createCharacter`/`updateCharacter` used by CharacterScene/WorldScene/RoomScene, running the shared idle-bob loop continuously (no walk/flip needed here — it's a static display, not a controllable character).
- **Default landing page:** `LoginScene` currently routes straight to `WorldScene` (returning players) or `CharacterScene` (new players, no `characterConfig` yet). That now becomes: new players still go `LoginScene → CharacterScene` first (a profile needs a character to show), then everyone — new and returning — lands on `ProfileScene` before the world. `ProfileScene` gets an **"Enter World"** button that starts `WorldScene`.
- **Return button:** `WorldScene` gained a "My Profile" HUD button that pauses `WorldScene` (never stops it) and launches `ProfileScene` on top in self mode — same `scene.pause`/`scene.launch` technique already used for `GameScene` on top of a paused `RoomScene` (Phase 15), so the live Colyseus connection is preserved rather than dropped and rejoined. "Back" resumes `WorldScene` instead of a fresh join.
- **Shared route, self vs. viewing another player:** `ProfileScene.init()` takes an optional `uid`.
  - No `uid` (or own `uid`) → **self mode**: own creations/processLog/feedback, "Enter World" button, editable where the underlying data already supports owner-only edits (e.g. Process Log).
  - Foreign `uid` → **viewing mode**: read-only, "Back" button returns to wherever the viewer came from instead of "Enter World".
- **Navigating to another player's profile (decided 2026-07-24) — two entry points:**
  1. **Click their sprite/nametag** in `WorldScene` (or any shared room) — only reaches players physically co-present in the same Colyseus room instance as the viewer, same as today's presence model.
  2. **"Players Online" sidebar** in `WorldScene` — a roster of *every* currently-connected player app-wide, not just those in the viewer's own room instance, each entry clickable straight into `ProfileScene` viewing mode. This is the only way to reach a player who's connected but not co-present (see below), and needs a new **cross-room online roster** on the server: `WorldRoom` and `SessionRoom` instances are isolated from each other by design (Phase 15 — "players in a session see only each other"), so nothing today aggregates presence across both. Cheapest implementation: a small endpoint/broadcast built directly off the existing `worldRoomInstance`/`sessionRoomInstances` singletons (Phase 15) reading each room's `state.players`, rather than any new Firestore-backed presence store.
  - **Session tagging:** a player connected via an active `SessionRoom` is still listed in the sidebar for main-world viewers (their profile stays reachable), but tagged with a badge — e.g. "In-Person Session" — reusing the same distinction as the existing per-player top-left "Main World"/"In-Person Session" badge from Phase 15, just shown to *other* players about that player's context instead of their own.
- Reads `creations`, `processLog`, and `feedback` filtered by `uid` — same data sources as originally scoped, no new data model.
- Visible to everyone by default; `users/{uid}.profileVisible` remains an **admin-only** toggle — when off, viewing mode shows a "this profile is hidden" state for other players; never affects the owner's own self-mode view or their post-login landing.
- Built after Phases 12–14 since it's an aggregation view over data those phases produce.
...
## Deferred Follow-Ups (revisit once the baseline draft — Phases 1–17 — is complete)

A running list of things intentionally left narrower than they could be, or
left unverified, so they can all be triaged in one pass later instead of
getting lost across individual phase sections. Owner's stated intent: finish
a full baseline draft first, then sweep this list at the end.

**Not yet built (scope deferred):**
- Editing/resubmitting an interactive object's code — currently create-only (Phase 11)
- Drag-move for interactive objects — needs a new `onMove(scene, ctx, x, y)`-style contract hook, since unlike decorative objects there's no single engine-owned `Container` to grab (Phase 11)
- `linkedArtifacts` support on interactive objects — decorative-only today; an object's own `onInteract` can already `window.open()` a link itself if the author wants that (Phase 11)
- Interactive-object code sharing/remix (Phase 13) — would need (1) an edit/reopen flow for interactive objects (same gap as above, since sharing requires something to toggle after approval) and (2) an identifying label for an object, since objects have no `name` field today the way rooms do (the remix picker currently falls back to a version's description or "object by &lt;displayName&gt;")

**Already built, not yet live-verified (testing deferred, not scope):**
- Game and music sharing/remix (Phase 13) — implemented identically to room sharing (same `buildSharingFieldsHTML`/`wireShareCheckbox` code path, no `includeShare:false` exclusion), `curl`-verified server-side, but only room sharing has been walked through live in-browser so far. Owner's explicit call: keep the feature as built, just verify it whenever convenient rather than stripping it back to room-only.
