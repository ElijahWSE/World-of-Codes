...
- **M3 — Ownership tightens (9B): ✅ complete (2026-07-16).** New claims carry `uid` end-to-end. `POST /admin/link-owner {password, slotKey, uid}` binds a legacy slot to a real account. `/admin/pending` computes a non-blocking `ownerMismatch` boolean per submission (linked slot `uid` vs. submission `uid`) and `admin.html` shows an "⚠ OWNER MISMATCH" badge — verified live, does not block approval.
