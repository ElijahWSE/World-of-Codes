...
### PHASE 16 — PROFILE PAGE (BUILT 2026-07-24, core UI live-verified by the owner — not yet fully closed out)
...
**Build status:** all pieces below are implemented and the client/server build clean (`vite build`, `node --check`). The owner live-verified the landing flow and iterated on `ProfileScene`'s layout directly in-browser — two real UI bugs were found and fixed on the spot: (1) the original ground-"glow" behind the character was two overlapping flat Phaser circles, which reads as a visible ring/bullseye rather than a soft light (Phaser's basic shapes have hard edges, no blur) — replaced with a single flat contact-shadow ellipse at the character's feet; (2) the character preview sat too far left relative to the newly-right-docked info panel — `PREVIEW_X`/`PREVIEW_Y` retuned (210→280, 370→380) to better center it in the visible canvas area. **Not yet explicitly exercised live:** the in-world "My Profile" HUD button's pause/launch round-trip (does it really preserve the live Colyseus connection with no visible reconnect), viewing another player via sprite/nametag click or the "Players Online" sidebar, the sidebar's in-person-session tagging, and the admin `profileVisible` hide/unhide toggle. Treat those as unverified until explicitly checked, per this project's usual bar for calling a phase closed out.
...
- **Return button:** `WorldScene` gains a new HUD button/hotkey ("My Profile") that stops `WorldScene` and starts `ProfileScene` in self mode — mirrors the existing `[H]`/`[P]`/`[F]` overlay convention in RoomScene, but as a real scene swap rather than a DOM overlay, since leaving `WorldScene` means leaving the multiplayer room. Exact key TBD at build time — `E` and `U` are already claimed in `WorldScene`.
