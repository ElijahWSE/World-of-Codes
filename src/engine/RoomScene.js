...
export default class RoomScene extends Phaser.Scene {
  ...
  // ── create ────────────────────────────────────────────────────────────────────
  create() {
    // Fallback background — rooms draw their own floor on top of this.
    this.add.rectangle(400, 300, 800, 600, 0x111118);

    // ── Player sprite ─────────────────────────────────────────────────────────
    // Exposed as `scene.player` so room hooks can reference it (e.g. for
    // collision or overlap checks). Spawns at screen center.
    this.player = this.add.rectangle(400, 300, 32, 32, 0x00cc44).setDepth(10);
    this.physics.add.existing(this.player);
    this.player.body.setCollideWorldBounds(true); // can't walk off the 800×600 canvas

    // Name tag — repositioned every frame to float above the player
    this._nameTag = this.add.text(400, 276, this._playerName, {
      fontSize: '12px',
      fill: '#ffffff',
      stroke: '#000000',
      strokeThickness: 3,
